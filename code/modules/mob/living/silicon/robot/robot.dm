...
/mob/living/silicon/robot/get_status_tab_items()
	. = ..()
	if(cell)
		. += text("Charge Left: [round(cell.percent())]%")
		. += text("Cell Rating: [round(cell.maxcharge)]") // Round just in case we somehow get crazy values
		. += text("Power Cell Load: [round(used_power_this_tick)]W")
	else
