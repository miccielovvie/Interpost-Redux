...
/mob
	...
	var/list/trained_skills = list()
	var/list/trained_stats = list()
...
/mob/proc/learn_stats(var/stat_type)
	...
	if(stats[stat_type] > 15)
		...
		stats[stat_type] += 0.01 * learn_speed
	if(round(stats[stat_type]) > initial_stat)
		if(ishuman(usr))
			var/mob/living/carbon/human/H = usr
			H.add_event(stat_type, /datum/happiness_event/experience)
			H.trained_skills |= stat_type
		//Basically what this is doing it is keeping the skill where it is.  This allows out check 
		//in the "sleep" verb to hand increasing the skill, and this only gives up the effect and the stat to increment.
		stats[stat_type] = initial_stat
...
/mob/proc/learn_skills(var/skill_type)
	...
	if(round(skills[skill_type]) > initial_skill)
		if(ishuman(usr))
			var/mob/living/carbon/human/H = usr
			H.add_event("experienced",/datum/happiness_event/experience)
			H.trained_skills |= skill_type
		//Basically what this is doing it is keeping the skill where it is.  This allows out check 
		//in the "sleep" verb to hand increasing the skill, and this only gives up the effect and the stat to increment.
		skills[skill_type] = initial_skill
